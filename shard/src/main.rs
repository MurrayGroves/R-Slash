...
use serenity::all::{
    ...
    CreateCommand, Emoji, GuildId, ReactionType,
};
...
use rslash_common::{
    ConfigStruct, InteractionResponse, InteractionResponseMessage, ResponseFallbackMethod,
    ...
    U: Into<u64> + Debug + Send + Clone + 'static,
{
    ...
    tokio::spawn(async move {
        ...
        let config = data.read().await.get::<ConfigStruct>().unwrap().clone();
...
impl EventHandler for Handler {
    ...
    async fn guild_create(&self, ctx: Context, guild: Guild, is_new: Option<bool>) {
        ...
        if let Some(x) = is_new {
            ...
            if x {
                let config = ctx.data.read().await.get::<ConfigStruct>().unwrap().clone();
                ...
                if let Some(variant) = feature_flags.get("help_command") {
                    if variant == "enabled" {
                        let _ = guild
                            .create_command(
                                &ctx.http,
                                CreateCommand::new("help").description("Learn how to use the bot"),
                            )
                            .await;
                    }
                };
