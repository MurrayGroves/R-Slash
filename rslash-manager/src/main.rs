...
async fn get_redis_connection() -> redis::Connection {
    ...
    let ip = ip_json.expect("Failed to convert string to json")["items"][0]["status"]["addresses"]
        [0]["address"]
        .to_string()
        .replace('"', "");
...
async fn get_discord_details() -> (u64, u64) {
    ...
    let client_k8s = kube::Client::try_default().await.unwrap();
    let credentials: kube::Api<k8s_openapi::api::core::v1::Secret> =
        kube::Api::namespaced(client_k8s, &namespace);
    ...
    let total_shards: u64 =
        serde_json::from_value::<u64>(json["shards"].clone()).expect("Gateway response not u64");
    let max_concurrency: u64 =
        serde_json::from_value::<u64>(json["session_start_limit"]["max_concurrency"].clone())
            ...
            match { env::args_os().nth(3) } {
                ...
                Some(x) => {
                    ...
                    rollout
                        ...
                        .arg(namespace)
                        .arg("image")
                        .arg("statefulset/discord-shards")
                        .arg(format!(
                            "discord-shard=registry.murraygrov.es/discord-shard:{}",
                            tag
                        ));
            ...
            match { env::args_os().nth(3) } {
                ...
                Some(x) => {
                    ...
                    rollout
                        ...
                        .arg(namespace)
                        .arg("image")
                        .arg("deployment/discord-interface")
                        .arg(format!("discord-interface=discord-interface:{}", tag));
                    let output = rollout.output().expect("Failed to run kubectl");
                    println!("{:?}", String::from_utf8(output.stdout).unwrap());
                    println!("{:?}", String::from_utf8(output.stderr).unwrap());
                }
            ...
            match { env::args_os().nth(3) } {
                ...
                Some(x) => {
                    ...
                    rollout
                        ...
                        .arg(namespace)
                        .arg("image")
                        .arg("deployment/reddit-downloader")
                        .arg(format!("reddit-downloader=reddit-downloader:{}", tag));
                    let output = rollout.output().expect("Failed to run kubectl");
                    println!("{:?}", String::from_utf8(output.stdout).unwrap());
                    println!("{:?}", String::from_utf8(output.stderr).unwrap());
                }
